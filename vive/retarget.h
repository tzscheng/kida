...
// DG5F-S - initially uses the same 20-dof mapping as DG5F-M.
inline void RetargetSide_dg5s(char side, const float _q[20], float* q)
{
    if (side == 'L') {
        q[0]  =  0.5f * _q[1]  - 0.5f;
        q[1]  =  1.6f * _q[0]  + 0.5f;
        ...
        q[12] =  1.0f * _q[12] + 0.2f;
        q[13] =  1.2f * _q[13] + 0.0f;
        q[14] =  1.2f * _q[14] + 0.0f;
        q[15] =  1.2f * _q[15] + 0.0f;

        if (_q[17] > 0.95 && _q[18] > 0.44) {
            q[16] =  1.5f * _q[16] + 0.0f;
        } else {
            q[16] =  0.3f * _q[16] + 0.0f;
        }
        q[17] =  1.2f * _q[16] + 0.2f;
        q[18] =  1.2f * _q[18] + 0.1f;
