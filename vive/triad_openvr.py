...
class vr_tracked_device():
    ...
    def _pose_ok(self, p):
        # bPoseIsValid 만 보면 부족: base station 이 트래커를 못 볼 때 OpenVR 은
        # bPoseIsValid=True 를 유지하면서 eTrackingResult 를 Running_OutOfRange
        # 로 바꾸고 position 은 IMU dead-reckoning 추정값을 돌려준다. 이 추정은
        # 수 cm ~ 수십 cm 드리프트했다가 트래킹 회복 시 'snap' 한다. vive-udp.cpp
        # 와 동일한 3단계 필터로 거른다.
        return (p.bDeviceIsConnected
                and p.bPoseIsValid
                and p.eTrackingResult == openvr.TrackingResult_Running_OK)

    def get_pose_euler(self, pose=None):
        if pose == None:
            pose = get_pose(self.vr)
        if self._pose_ok(pose[self.index]):
