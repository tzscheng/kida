...
- `kida.run` / `single.run` — executable runners (uv shebang) that replace the generic `tact/start`. Unlike `start`, these wire up an arm agent **plus** one or two hand agents (`h9` or `dg5`) and concatenate the `u`/`y` vectors themselves.
...
These runners do **not** use `tact/start`'s logging/dispatch. They do their own ZMQ binding:
...
- **external PID** (cmode==0 or chrono/PWM-only HW): `pid.update(...) + m.gravity(q)` for joint targets, or `jtc.update(...) + m.gravity(q) + sk*(rq - q)` for task targets — the `sk*(rq - q)` term is a soft postural spring biasing the elbow.
