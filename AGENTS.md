...
`kida` is a control project for a 14-DOF dual-arm manipulator (two 7-DOF arms sharing a torso `root`). It lives at `/home/ubuntu/kida` as an independent repo, but it is not a standalone framework — it sits on top of the sibling `tact` toolkit (developed in its own repo `pytact` at `github.com/tzscheng/pytact`, checked out at `../pytact` for reference) which it consumes as a **versioned PyPI release** (`pytact>=0.1.0a9` in `pyproject.toml`, from <https://pypi.org/project/pytact/>) for kinematics/dynamics, simulation, and rendering, and on the local `h9.py` or `dg5.py` gripper controller. There is deliberately **no** `[tool.uv.sources]` override for the local `../pytact` clone: edits there never leak into kida — publish a new pytact release and bump the version pin (then `uv sync`) to pick it up. `h9.py` and the H9 gripper YAMLs under `yaml/` are local snapshot copies of their fg originals (`../fg/h9.py`, `../fg/h9/yml/`) — they do not auto-track fg updates; re-copy when the originals change. See the `pytact` source at `../pytact` for the framework details (that repo no longer tracks its own `AGENTS.md`).

This project provides:

- `kida.py` — `Controller` class for the full dual-arm (14 joints, `n_u=14`, `n_y=42`).
- `single.py` — `Controller` class for one arm (`n_u=7`, `n_y=21`), used when running just the left or right side.
- `kida-run` / `single-run` — executable runners (uv shebang) that replace the generic `fg/start`. Unlike `start`, these wire up an arm controller **plus** one or two hand controllers (`h9` or `dg5`) and concatenate the `u`/`y` vectors themselves.
- `eio/eio-kida.c` / `eio/eio-single.c` → `eio/eio-kida.so` / `eio/eio-single.so` — C shared libs that drive the real arms over CAN (per-arm pthread on cores 1, 2) and the DG-5 hands over UDP (`127.0.0.1:6660` left, `:6661` right). `htype=1` forks `eio/eio-dg5f`, `htype=2` forks `eio/eio-dg5s`, and `htype=0` is H9-style hands handled inside the Python loop.
- `yaml/kida.yaml`, `yaml/kida-left.yaml`, `yaml/kida-right.yaml` — robot models for the dual arm and per-arm variants. Gripper YAMLs (`dg5f-*.yaml` for the DG-5F-M, `dg5s-*.yaml` for the smaller DG-5F-S, `h9-*.yaml`) are local files (h9 ones copied from `../fg/h9/yml/`). The `dg5s-*` pair also declares 5 fingertip `tactiles:` (18 taxels each); the runners publish those generically off `env.tactiles`, so nothing gripper-specific is needed for them.
- `usrsample.py` — reference ZMQ client. `utils/rcvpp.py` is a minimal proprio subscriber.
...
./kida-run -g 1                              # tact sim, DG-5F-M hands (yaml/dg5f-*)
./kida-run                                   # tact sim, DG-5F-S hands (default: -g 2)
./kida-run -g 2                              # tact sim, DG-5F-S hands (yaml/dg5s-*)
...
./single-run -t 0                            # left arm + DG-5F-S hand, sim (default: -g 2)
```

Flags shared by both runners: `-x` real hardware (loads `eio/eio-*.so`), `-b` actuator built-in position controller, `-v` verbose, `-g 0|1|2` gripper (h9 | dg5f = DG-5F-M | dg5s = DG-5F-S; default `2`). `kida-run` also has `-m` (MuJoCo, currently commented out). `single-run` requires `-t 0|1` for left/right.

`-g` selects the YAML prefix **and** the TCP mount offsets `(hx, hy)` in one branch — the two are not independent, since each hand YAML has its own palm origin. `hx` is the wrist→palm gap, `hy` the lateral correction (negated on the right, where the +90 mount yaw flips tcp-y). For `dg5f`, `hy=0.0071` cancels a root-origin shift (both hands' 3 central finger bases sit at root-x=-0.0071) and is verified on render/HW. For `dg5s`, `(0.06, 0.015)` was set **by eye on the render, not derived** — its finger bases sit at root-x=+0.00245, so the same cancellation does not apply; re-check it if the S palm geometry changes. Note also that `hx` for `dg5f` differs between the runners (0.02 in `kida-run`, 0.05 in `single-run`) for an identical tcp frame — a pre-existing discrepancy, left as is.

There are deliberately no model-specific `dg5f.py`/`dg5s.py` modules: both hands have the same 20-joint layout and `dg5.py` holds no model-specific geometry (the model comes from `tact.Model(ymlname)`, and the home poses key off the `-left`/`-right` suffix), so both runners import it for `-g 1` and `-g 2`. On real hardware the eio hand type remains distinct so it can fork `eio-dg5f` or `eio-dg5s`.
...
- PUB CONFLATE `ipc:///dev/shm/<sensor_name>` — frames returned by `env.camera_frames()`/`env.tactile_frames()` for YAML-declared cameras and tactiles. Sim only; on real hardware camera frames come from a separate `rs2/msender` process.
...
## Controller state machine (kida.py / single.py)

Both `Controller` classes share the same `msgproc(w) → shift(state)` + `update(y) → (tau, q_ref, qd_ref, kp, kd)` shape. Recognised commands:
